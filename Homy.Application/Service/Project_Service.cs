    ...
    public class Project_Service : IProject_Service
    {
        ...
        public async Task<bool> DeleteProjectAsync(long id)
        {
            ...
            if (propertiesCount > 0)
                throw new InvalidOperationException($"?? ???? ??? ??????? ????? {propertiesCount} ???? ?????? ??");
        ...
        public async Task<ProjectStatsDto> GetProjectStatsAsync()
        {
            ...
            int totalUnits = 0;
            foreach (var project in allProjectsList)
            {
            ...
            {
                TotalProjects = allProjectsList.Count,
        ...
        /// <summary>
        /// Helper Method: ????? Project ??? ProjectDto
        /// </summary>
